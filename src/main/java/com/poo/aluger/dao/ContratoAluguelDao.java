...
public class ContratoAluguelDao implements GenericDao<ContratoAluguel> {
    ...
    public boolean insert(ContratoAluguel contratoAluguel) throws SQLException, IOException, ClassNotFoundException {
        try(Connection connection = DBConnector.getConnection()) {
            ...
            stmt.setDate(2, contratoAluguel.getDataTermino() != null ? java.sql.Date.valueOf(contratoAluguel.getDataTermino()) : null);
    ...
    @Override
    public boolean update(int id, int codigoProprietario, ContratoAluguel contratoAluguel) throws SQLException, IOException, ClassNotFoundException {
        try(Connection connection = DBConnector.getConnection()) {
            PreparedStatement stmt = connection.prepareStatement(
    ...
    public ContratoAluguel findById(int id, int codigoProprietario) throws SQLException, IOException, ClassNotFoundException {
        try(Connection connection = DBConnector.getConnection()) {
            PreparedStatement stmt = connection.prepareStatement(
                    "SELECT * " +
            ...
            if(rs.next()) {
                ...
                LocalDate CA_DataTermino = rs.getDate("CA_DataTermino") != null ? rs.getDate("CA_DataTermino").toLocalDate() : null;
                ...
                Imovel imovel = new Imovel(I_Codigo, I_Foto, I_Rua, I_Numero, I_Bairro, I_Cidade, I_Estado, I_TipoImovel, I_AreaTotal, I_QtdQuartos, I_Status, I_QtdBanheiros, I_Descricao, proprietario);
                Inquilino inquilino = new Inquilino(INQ_Codigo, INQ_Nome, INQ_Cpf, INQ_Telefone1, INQ_Telefone2);

                return new ContratoAluguel(CA_DataInicio, CA_DataTermino, CA_ValorAluguel, CA_DiaPagamento, inquilino, imovel, proprietario);
            }
            return null;
        }
    }

    public List<ContratoAluguel> findAll(int codigoProprietario) throws SQLException, IOException, ClassNotFoundException {
        try(Connection connection = DBConnector.getConnection()) {
            PreparedStatement stmt = connection.prepareStatement(
                    "SELECT * " +
                        "FROM ContratoAluguel " +
            ...
            while(rs.next()) {
                ...
                ContratoAluguel contrato = new ContratoAluguel(CA_DataInicio, CA_DataTermino, CA_ValorAluguel, CA_DiaPagamento, inquilino, imovel, proprietario);
